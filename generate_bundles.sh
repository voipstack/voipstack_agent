...
generate_debian_bundle() {
    local output_file="$OUTPUT_DIR/bundle.install.debian.sh"
...
EOF
    ...
    chmod +x "$output_file"
    echo "Generated Debian bundle: $output_file"
}

generate_centos7_bundle() {
    local output_file="$OUTPUT_DIR/bundle.install.centos7.sh"

    cat > "$output_file" << 'EOF'
#!/bin/bash
# AI-Assisted
# VoIPStack Agent - CentOS 7 Bundle Installation Script
...
EOF
    ...
    chmod +x "$output_file"
    echo "Generated CentOS 7 bundle: $output_file"
}

main() {
    echo "Generating bundle installation scripts..."
    echo ""
    ...
    echo "  - bundle.install.debian.sh   (for Debian/Ubuntu systems)"
    echo "  - bundle.install.centos7.sh  (for CentOS 7 systems)"
    echo ""
    echo "Usage:"
    echo "  sudo ./bundle.install.debian.sh <AGENT_TOKEN>"
    echo "  sudo ./bundle.install.centos7.sh <AGENT_TOKEN>"
}
